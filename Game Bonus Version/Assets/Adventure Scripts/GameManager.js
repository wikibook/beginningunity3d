...
internal var showMOCursorChange : boolean; //flag for changing the cursor color
internal var showPointer : boolean; // flag for pointer visibility 
...
internal var gridPosition: int = 0; // the default position of the inventory grid
internal var iLength : int; // length of the array of inventory objects 
...
internal var end : boolean = false;  //end sequence 
internal var camMatch : boolean = false; // flag to hide mouse during camera match
...
internal var currentInventoryObjects = new Array(); 

// array to hold the action objects in case they need to be reactivated
internal var actionObjects = new Array();
...
internal var playerSettings  = new Array (); // player settings array

private var retrig : boolean = false; 
...
function Start () {
   ...
   Screen.showCursor = false; // hide the os cursor
	...
	yield; // **********************************************
	UpdateCurrentInventory (); // update inventory icon staus
	...
	if (Input.GetMouseButtonDown(0)) { // if the left button, 0, is pressed
		...
		if (didHit) { // you did hit something
		   if (hit.transform.tag == "ActionObject") return; // do nothing
			  ...
			  cursor.GetComponent(Interactor).currentState = 1; // update current state ****** moved this up
...
function OnGUI () {
   ...
   if (useText){  //global toggle
      ...
      if (showText && !showActionMsg){ //local toggle
   	...
   	if (end) return; // no more game gui needed *********moved this
   ...
   if (resetMO) GUI.Box (Rect (0, 0, Screen.width,Screen.height),"",customGUIStyle);
	...
	if (retrig == true) GUI.Box (Rect (0,0,Screen.width, Screen.height ), "");
...
function AddToInventory (object : GameObject) {
   ...
   // add the new object to the current inventory array
	...
	//check for any special cases when putting into inventory ***************************
	CheckSpecialCases(object);
...
function CheckSpecialCases (object : GameObject) {

	if (object.name ==  "Light Glyph Icon") { // turn on the light while it is as cursor, else turn it off
		var tempState : int = object.GetComponent("Interactor").currentState;
		if ( tempState == 2) {
		gameObject.Find("torch Light FX").GetComponent(Light).enabled = true;
		}
		else {
			gameObject.Find("torch Light FX").GetComponent(Light).enabled = false;
			gameObject.Find("torch Light shrine").GetComponent(Light).enabled = false; // this one is with glyph object
		}
		//print("lighting" + "  " + tempState);
	}

}

function ReTriggerMO () {
