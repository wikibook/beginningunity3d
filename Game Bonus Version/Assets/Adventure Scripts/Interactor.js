...
var dropType : int = 0; // 0 is not dropable, 1 is dropable , 2 is a drop location, 3 uses a rigidbody, so needs transformed saved
...
internal var processing = false; //so we can suspend mouseover actions, etc
...
internal var iElement = 0; // will hold the object's element in inventory number  
...
private var menuMode : boolean; // menu mode flag
...
internal var soundFXVolume : float; // we will get this from GameManager
private var loading = false; // flag for alternate processing of object state
...
function Start () {
    ...
    if(objectIs3D) { // do the following
		// load the material into the var
		if(GetComponent(MeshRenderer))
		   originalMaterial = GetComponent(MeshRenderer).material; 
		if(GetComponent(SkinnedMeshRenderer))
		   originalMaterial = GetComponent(SkinnedMeshRenderer).material;
...
function OnMouseDown () {
	...
	// allow picks from layers 8 and 9 in Inventory mode only
...
function DistanceFromCamera () {
 ...
 function ProcessObject (newState : int) {
  ...
  //print("processing " + currentObjectName);
   currentObjectName = objectName[currentState];
   currentObjectDescription = description[currentState];
   currentLocation = location[currentState];
   currentVisibility = visibilityType[currentState];
	...
	if (dropType == 1) {
	   ...
	   transform.position = holder.transform.position;
		// rotate only if matching an object instead of a dropbox location
		if (holder.transform.localScale == Vector3(1,1,1)) 
   ...
   if(currentSound && !loading)  { // if there is a sound and the state is not being loaded 
